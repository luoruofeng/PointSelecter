...
const pickDensityRange = document.getElementById('pick-density-range');
    ...
    palette.forEach((c, idx) => {
        ...
        col.innerHTML = `
            <div class="form-check d-flex align-items-center gap-2">
                <input class="form-check-input color-choice" type="checkbox" id="color-${idx}" ${isLight ? 'disabled' : ''}
...
function handlePickClick(e) {
    if (!pickModeActive) return;
    const p = getPixelColorAtClientEvent(e);
    const density = parseInt(pickDensityRange.value, 10);
    addRegionPoints(p.x, p.y, { r: p.r, g: p.g, b: p.b }, density);
}
